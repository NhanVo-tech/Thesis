...
\begin{enumerate}
    ...
    \item \textbf{Secure Ranging \& Localization (Đo khoảng cách và định vị an toàn - Phase C)}
    \begin{itemize}
        \item Hệ thống tự động kích hoạt phiên đo khoảng cách UWB \textbf{đa mỏ neo} (ba mỏ neo) ngay sau khi xác thực BLE thành công, thông qua lệnh điều khiển trên đường hầm CCC (\texttt{RANGING\_START/STOP}).
        \item Sử dụng giao thức Symmetrical Double-Sided Two-Way Ranging (DS-TWR) chế độ \textbf{multicast} (một bộ khởi tạo, ba bộ đáp ứng) để loại bỏ sai số trôi đồng hồ (Clock drift) giữa thiết bị di động và các mỏ neo. Ba khoảng cách đồng thời $d_0, d_1, d_2$ được hợp nhất bằng thuật toán Trilateration để xác định tọa độ phẳng $(x, y)$ của thiết bị trong hệ quy chiếu xe.
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{UWB Subsystem:} Sử dụng \textbf{ba mỏ neo UWB} cố định (ba bo mạch DWM3001CDK chạy vi mạch Qorvo DW3000) gắn trên thân xe tại các vị trí $A_0$ (cản sau), $A_1$ (bên phải) và $A_2$ (cột B trái). Các mỏ neo hoạt động ở chế độ Responder, thực hiện đo khoảng cách an toàn (Distance Bounding) với điện thoại (Controller) theo giao thức DS-TWR multicast. Dữ liệu khoảng cách được một \textbf{cầu nối PC} (chạy script Python) thu thập qua USB/UART từ ba mỏ neo, đóng gói thành khung \texttt{RANGE} và truyền sang ESP32 qua kênh USB-CDC. Toàn bộ tải tính toán UWB được đẩy khỏi vi điều khiển, ESP32 chỉ đảm nhận việc hợp nhất dữ liệu (Trilateration + EKF) và ra quyết định.
...
\begin{itemize}
    ...
    \item \textbf{Foreground Isolate (Luồng giao diện \& UWB Ranging):} Xử lý dịch vụ \texttt{UwbMultiService} thông qua Native Android MethodChannel (\texttt{smartcar/uwb}). Kế thừa kết nối BLE từ Background, luồng này gửi lệnh \texttt{RANGING\_START/STOP} qua đường hầm CCC đã mã hóa cho ESP32, đồng thời kích hoạt chip UWB trên điện thoại (API \texttt{android.ranging}, địa chỉ Controller \texttt{0x06C1}) để bắt đầu phiên đo khoảng cách \textbf{DS-TWR multicast} với ba mỏ neo trên xe.
    \item \textbf{Security \& ML Module:} Chứa module nhận diện ý định hành vi \textit{TrajInference} (Conv1D/TFLM, \textbf{đang hiện thực}). Module này phân loại cửa sổ quỹ đạo $(x, y, v_x, v_y)$ thành bốn lớp ý định (Approach/Seated/Leave/Passing) và đóng vai trò cổng (gate) cho quyết định truy cập tất định.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{UWB Task:} Đọc luồng khung dữ liệu \texttt{RANGE:d0,d1,d2,valid} từ kênh USB-CDC do cầu nối PC gửi sang, phân tích cú pháp, đưa qua thuật toán \textbf{Trilateration} để giải tọa độ $(x, y)$, cập nhật \textbf{Bộ lọc Kalman hai chiều (EKF)} để ước lượng trạng thái $(x, y, v_x, v_y)$, rồi bàn giao cho khối quyết định truy cập \texttt{AccessController}.
    \item \textbf{Access Controller (Máy trạng thái truy cập + Geofencing):} Đảm nhiệm quyết định truy cập hình học. Module \texttt{Geofence} phân lớp vị trí $(x, y)$ thành các vùng ảo (\texttt{DRIVER\_SEAT}, \texttt{DRIVER\_DOOR}, \texttt{WELCOME}, \texttt{OUTSIDE}) với bộ đệm Hysteresis. Trên nền các vùng này, module \texttt{AccessController} triển khai máy trạng thái ba trạng thái (\texttt{LOCKED}, \texttt{DOOR\_UNLOCKED}, \texttt{OCCUPIED}): mở khóa khi người dùng giảm tốc gần như dừng ($<0.4$ m/s) gần cửa trong 3 chu kỳ liên tiếp, khóa cửa và cấp quyền khởi động (GPIO27) khi người dùng đứng yên ở ghế lái 3 giây, rồi khóa xe và thu hồi quyền khi người dùng rời đi.
...
\textbf{Foreground Isolate:} Nhận Handoff, gọi API Native Android cấu hình chip UWB và gửi lệnh Ranging. & \textbf{Tasks \& Protocol Layer:} NFC Task (APDU), BLE Task (GATT/Crypto), UWB Task (Trilateration + EKF). \\ \hline
\textbf{ML Layer:} TrajInference (Conv1D/TFLM) nhận diện ý định hành vi, đóng vai trò cổng cho quyết định (đang hiện thực). & \textbf{Access Logic:} Thuật toán Hysteresis kết hợp dữ liệu Ranging để điều khiển GPIO Relay. \\ \hline
\end{tabular}
...
\begin{enumerate}
    ...
    \item \textbf{Phase C: Xác thực khoảng cách và định vị (Physical Distance Bounding \& Localization) - UWB} \\
    Sau khi Phase B thành công, điện thoại gửi lệnh \texttt{RANGING\_START} qua đường hầm CCC. ESP32 ra lệnh cho cầu nối PC khởi động phiên đo trên ba mỏ neo. Giao thức DS-TWR (Double-Sided Two-Way Ranging) multicast hoạt động, liên tục trả về ba khoảng cách ($d_0, d_1, d_2$) được hợp nhất thành tọa độ $(x, y)$ qua Trilateration và làm mượt bằng EKF.
    
    \item \textbf{Phase D: Quyết định truy cập (Access State Machine)} \\
    Vị trí $(x, y, v_x, v_y)$ sau EKF được đẩy vào khối \texttt{Geofence} để phân vùng và bàn giao cho máy trạng thái \texttt{AccessController}. Máy trạng thái chuyển \texttt{LOCKED} $\rightarrow$ \texttt{DOOR\_UNLOCKED} khi người dùng giảm tốc gần như dừng ($<0.4$ m/s) ở vùng cửa trong 3 chu kỳ liên tiếp; chuyển \texttt{DOOR\_UNLOCKED} $\rightarrow$ \texttt{OCCUPIED} (khóa cửa + cấp quyền khởi động) khi người dùng đứng yên ở ghế lái 3 giây; và chuyển về \texttt{LOCKED} khi người dùng rời đi, đồng thời FSM phải đang ở trạng thái \texttt{AUTH\_SECURE\_CHANNEL\_READY} (Đã xác thực BLE) thì ESP32 mới cấp điện cho GPIO kích hoạt Relay.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Peripherals:} Hỗ trợ ma trận phần cứng linh hoạt, cho phép cấu hình UART độc lập (UART2 cho PN532) và kênh USB-CDC bản địa (native USB) để nhận luồng dữ liệu UWB từ cầu nối PC mà không xung đột tài nguyên.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Protocol Layer:} Xử lý APDU, BLE GATT, RANGE Parser (USB-CDC). Tuân thủ tiêu chuẩn quốc tế.
...
\subsection{Phase C - Xác thực khoảng cách và định vị an toàn với UWB}
Khác với các hệ thống Smart Key truyền thống dùng RSSI của Bluetooth để đo khoảng cách (vốn dễ bị khuyếch đại bởi các bộ lặp Relay), hệ thống sử dụng công nghệ UWB (Ultra-Wideband) làm thước đo vật lý chính xác tuyệt đối. Giai đoạn hai nâng cấp Phase C từ đo khoảng cách đơn mỏ neo lên \textbf{định vị đa mỏ neo}: ba khoảng cách đồng thời được hợp nhất thành tọa độ phẳng $(x, y)$, cho phép xác định chính xác vị trí tương đối của người dùng quanh xe.
...
\begin{enumerate}
    \item \textbf{Kích hoạt phiên Ranging:} Sau khi Phase B hoàn tất, điện thoại gửi lệnh \texttt{RANGING\_START} (INS \texttt{0x84}) qua đường hầm CCC đã mã hóa. ESP32 (lớp \texttt{UwbBridge}) dịch thành lệnh \texttt{CMD:START\_RANGING} gửi sang cầu nối PC qua kênh USB-CDC.
    \item \textbf{Khởi động phiên đa mỏ neo:} Cầu nối PC (script \texttt{run\_fira\_bridge.py}) khởi tạo phiên DS-TWR multicast trên cả ba mỏ neo DWM3001CDK ($A_0, A_1, A_2$) với bộ tham số FiRa chung (kênh 9, preamble 9, DS-TWR deferred, static STS). Điện thoại đóng vai trò Controller duy nhất (địa chỉ \texttt{0x06C1}), ba mỏ neo là Responder.
    \item \textbf{Đo khoảng cách đồng thời (Secure Ranging):} Trong mỗi vòng đo, điện thoại thực hiện giao thức \textbf{Symmetrical Double-Sided Two-Way Ranging (DS-TWR)} đồng thời với cả ba mỏ neo. Giao thức này triệt tiêu hoàn toàn sai số trôi đồng hồ (Clock drift) giữa điện thoại và từng mỏ neo. Cầu nối PC thu thập ba khoảng cách $d_0, d_1, d_2$, kiểm tra tính tươi (freshness) và phạm vi hợp lệ, rồi đóng gói thành khung \texttt{RANGE:d0,d1,d2,valid} gửi sang ESP32.
    \item \textbf{Định vị Trilateration và lọc Kalman:} ESP32 giải tọa độ $(x, y)$ từ ba khoảng cách bằng thuật toán \textbf{Trilateration} (nghiệm tuyến tính + tinh chỉnh Gauss-Newton), sau đó cập nhật \textbf{Bộ lọc Kalman hai chiều} để thu được trạng thái mượt $(x, y, v_x, v_y)$ ở nhịp 10 Hz.
...
\subsection{Phase D - Quyết định truy cập thông minh}

Đây là giai đoạn thực thi cuối cùng và quan trọng nhất của hệ thống. Việc kích hoạt rơ-le mở cửa (Door Unlock Actuator) không còn phụ thuộc vào ngưỡng khoảng cách vô hướng đơn lẻ từ một mỏ neo, mà là kết quả của sự \textbf{quyết định kết hợp} giữa trạng thái giao thức, vị trí không gian $(x, y)$ và vận tốc $(v_x, v_y)$ được hợp nhất từ ba mỏ neo. Lõi quyết định là tầng tất định \texttt{AccessController}; trên nền đó, mô hình TinyML nhận diện ý định hành vi (\textbf{đang hiện thực}) đóng vai trò \textbf{cổng} (gate) bổ sung.
...
Trạng thái $(x, y, v_x, v_y)$ sau khi đi qua bộ lọc Kalman hai chiều (EKF) sẽ được bàn giao cho module \texttt{AccessController} để thực hiện quyết định truy cập hình học, đồng thời được đưa vào module \texttt{TrajInference} (\textbf{đang hiện thực}). Tại đây, một cửa sổ trượt (Sliding Window) lưu trữ $25$ khung hình gần nhất sẽ thực hiện tính toán đặc trưng và suy luận ngay trên vi điều khiển (On-device Inference). Quy trình ra quyết định được thực hiện qua hai lớp:

\begin{enumerate}
    \item \textbf{Lớp quyết định tất định (Deterministic Core):}
    Module \texttt{AccessController} kiểm tra vùng (Geofence), cổng giảm tốc và máy trạng thái ba trạng thái như đã mô tả. Lớp này hoạt động độc lập với tầng AI và đảm bảo tính an toàn cơ học (fail-safe).

    \item \textbf{Lớp cổng ý định AI (Intent Gate, đang hiện thực):}
    Rơ-le cửa chỉ được phép kích hoạt \textbf{NẾU VÀ CHỈ NẾU} thỏa mãn đồng thời các điều kiện sau:
    \begin{itemize}
        \item \textbf{Giao thức:} FSM đang ở trạng thái \texttt{AUTH\_SECURE\_CHANNEL\_READY} (Đã xác thực BLE thành công).
        \item \textbf{Vật lý:} Khoảng cách Euclid từ vị trí đã lọc $(x, y)$ đến điểm mở khóa (cửa tài xế) nằm trong ngưỡng tiệm cận ($\le 2.0$ mét) trong ít nhất \textbf{3 chu kỳ đo liên tiếp}, đồng thời vận tốc hướng tâm không cho thấy người dùng đang rời xa (approach gate).
        \item \textbf{Ý định:} Mô hình \texttt{TrajInference} xác nhận ý định tiếp cận với độ tin cậy $p_{\text{approach}} \ge 0.80$. Ngược lại, nếu $p_{\text{passing}} \ge 0.70$ thì lệnh mở khóa bị \textbf{chặn cứng}, triệt tiêu các trường hợp người dùng chỉ đi ngang qua vùng nhận diện mà không có ý định vào xe.
    \end{itemize}
\end{enumerate}

\paragraph*{Điều kiện phục hồi (Cooldown):} 
Sau khi thực hiện lệnh mở cửa, hệ thống sẽ đi vào trạng thái cooldown. Cơ chế Reset chỉ được kích hoạt khi người dùng rời khỏi vùng thiết lập lại (Reset Zone, ví dụ khoảng cách Euclid đến điểm mở khóa $> 3.0$ mét), đảm bảo tính ổn định và tránh việc rơ-le hoạt động thừa thãi.
...
% =====================================================================
% [STAGE-1 AI - 1D RANGING] NỘI DUNG SAU ĐÂY ĐƯỢC COMMENT LẠI
% (walk/loiter/attack, residual, Kalman-LSTM) — thay thế bởi phần
% Stage-2 intent recognition ở cuối khối \iffalse ... \fi này.
% =====================================================================
\iffalse
...
\fi
% =====================================================================
% [STAGE-2 AI - 2D INTENT RECOGNITION] THIẾT KẾ (đang hiện thực)
% =====================================================================

\subsection{Cơ chế nhận diện ý định hành vi dựa trên TinyML (đang hiện thực)}

Trong các hệ thống PKE truyền thống, việc ra quyết định mở cửa chỉ dựa trên một ngưỡng khoảng cách tĩnh. Phương pháp này không phân biệt được người dùng đang chủ đích tiến vào xe hay chỉ đang đi ngang qua/lảng vảng gần xe. Để giải quyết bài toán này ở giai đoạn 2, kiến trúc hệ thống bổ sung một lớp nhận diện ý định sử dụng mô hình TinyML (Conv1D/TFLM) vận hành song song với bộ đệm Hysteresis. Cơ chế được thực hiện qua các bước sau:

\paragraph*{Cửa sổ thời gian và Trích xuất đặc trưng:}
Hệ thống duy trì một cửa sổ trượt (Sliding Window) có kích thước $T = 25$ khung hình (tương đương $2.5$ giây ở tần số 10 Hz). Tại mỗi khung hình, bộ tiền xử lý trích xuất vector đặc trưng $f_t = [x, y, v_x, v_y]$:
\begin{itemize}
    \item $x, y$: vị trí đã qua bộ lọc Kalman hai chiều (mét), mã hóa hình học cửa và ghế lái.
    \item $v_x, v_y$: vận tốc ước lượng (m/s), tín hiệu mạnh nhất cho tiếp cận/rời đi/đi ngang.
\end{itemize}

\paragraph*{Phân loại xác suất ý định:}
Ma trận dữ liệu đầu vào có kích thước $(25 \times 4)$ được chuẩn hóa z-score (tham số mean/scale từ tập huấn luyện) trước khi đưa vào mạng Conv1D trên vi điều khiển. Mô hình xuất ra phân bố xác suất Softmax cho 4 lớp ý định:
\begin{itemize}
    \item $p_{\text{approach}}$: xác suất người dùng đang chủ đích bước tới xe.
    \item $p_{\text{seated}}$: xác suất người dùng đã đứng yên tại ghế lái.
    \item $p_{\text{leave}}$: xác suất người dùng đang rời khỏi xe.
    \item $p_{\text{passing}}$: xác suất người dùng chỉ đi ngang qua xe.
\end{itemize}

\paragraph*{Tích hợp AI vào Logic điều khiển (Gate, don't decide):}
Xác suất đầu ra không thay thế cơ chế Hysteresis mà đóng vai trò màng lọc cấp cao (High-level Gate):
\begin{itemize}
    \item \textbf{Mở khóa:} cho phép khi người dùng nằm trong vùng tiệm cận ($\le 2.0$m) trong 3 chu kỳ liên tiếp và $p_{\text{approach}} \ge 0.80$.
    \item \textbf{Định cư ghế lái:} cho phép khi người dùng đứng yên ở ghế lái và $p_{\text{seated}} \ge 0.80$.
    \item \textbf{Rời đi:} cho phép khóa khi người dùng rời khỏi vùng chào đón và $p_{\text{leave}} \ge 0.80$.
    \item \textbf{Chặn mở khóa:} nếu $p_{\text{passing}} \ge 0.70$ thì chặn cứng lệnh mở khóa, triệt tiêu mở cửa sai khi đi ngang qua.
\end{itemize}

Kết hợp này tạo ra một kiến trúc "Phòng thủ chiều sâu" (Defense-in-depth), nơi thuật toán tất định đảm bảo an toàn cơ học, còn AI bổ sung độ bền vững ở mức ý định. Nếu mô hình không sẵn sàng (thiếu mô hình, tràn bộ nhớ), hệ thống tự suy biến về lõi tất định.
...
\begin{itemize}
  ...
  \item \textbf{AUTH\_SECURE\_CHANNEL\_READY}: Kênh BLE bảo mật đã thiết lập. Ở trạng thái này, hệ thống sẵn sàng nhận lệnh kích hoạt phiên UWB (\texttt{RANGING\_START} qua đường hầm CCC), nhưng \textbf{chưa được phép} bật rơ-le mở cửa.
\end{itemize}

\subsubsection{Nhóm trạng thái C\&D - Định vị và Kích hoạt relay}
Tiếp quản phiên làm việc từ Phase B để quyết định mở khóa:
\begin{itemize}
  \item \textbf{UWB\_RANGING\_ACTIVE}: Kích hoạt sau khi nhận được lệnh \texttt{RANGING\_START}. ESP32 gửi \texttt{CMD:START\_RANGING} sang cầu nối PC và liên tục nhận các khung \texttt{RANGE:d0,d1,d2,valid}, thực hiện Trilateration và cập nhật EKF để thu được vị trí $(x, y)$ theo giao thức DS-TWR đa mỏ neo.
  \item \textbf{UNLOCKING\_CHECK\_PROXIMITY}: Module \texttt{AccessController} thực hiện phân vùng (\texttt{Geofence}) và kiểm tra máy trạng thái truy cập trên vị trí $(x, y, v_x, v_y)$. Nếu người dùng giảm tốc gần như dừng ($<0.4$ m/s) trong vùng cửa 3 chu kỳ liên tiếp, kích hoạt sự kiện \texttt{PROXIMITY\_OK}.
