...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{image/section5/DWM3000EVB.png} % Cập nhật tên file ảnh mỏ neo UWB nếu cần
...
\subsection{Quyết định mở cửa theo vùng hình học (Phase D)}
Tọa độ $(x, y)$ sau EKF vẫn chứa dao động tự nhiên (Fluctuation) do cơ thể người dùng cản sóng. Nếu hệ thống kích hoạt Relay mở cửa ngay tại khoảnh khắc vị trí chạm giới hạn an toàn (ví dụ: cách điểm mở khóa $2.0$ mét), rơ-le sẽ bị đóng/ngắt liên tục (Relay Chattering).

Để giải quyết, đề tài triển khai module \texttt{AccessController} sử dụng màng lọc độ trễ trạng thái (Hysteresis/Debouncing logic) trên \textbf{khoảng cách Euclid} từ vị trí $(x, y)$ đến điểm mở khóa cửa tài xế $(-0.95, 0)$ (cột B bên trái, trùng với mỏ neo $A_2$), kết hợp thêm \textbf{cổng vận tốc hướng tâm} (radial-velocity approach gate) để từ chối các trường hợp người dùng đang rời xa hoặc đi ngang qua:

\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=C++, caption={Logic quyết định mở cửa theo vùng hình học (2D)}]
void AccessController::handlePosition(double x, double y,
                                      double vx, double vy) {
  double dx = x - kUnlockX, dy = y - kUnlockY;      // vector -> diem mo khoa
  double d  = sqrt(dx*dx + dy*dy);                  // khoang cach Euclid
  double vr = (vx*dx + vy*dy) / d;                  // van toc huong tam

  // 1. Reset neu nguoi dung buoc ra khoi vung Cooldown (> 3.0 m)
  if (d > RESET_RADIUS_M) {
    is_door_unlocked = false;
    consecutive_close_reads = 0;
    return;
  }
  // 2. Cua da mo thi giu nguyen trang thai
  if (is_door_unlocked) return;

  // 3. Cong van toc huong tam: dang roi xa -> huy bo dem
  if (vr > APPROACH_SPEED_MIN_MPS) {   // vr > 0.10 m/s = dang di ra xa
    consecutive_close_reads = 0;
    return;
  }

  // 4. Kiem tra vung mo khoa (<= 2.0 m)
  if (d <= UNLOCK_RADIUS_M) {
    if (++consecutive_close_reads >= REQUIRED_CONSECUTIVE_HITS) {
      fireRelayPulse();               // kich hoat GPIO26 trong 500 ms
...
Thuật toán yêu cầu khoảng cách Euclid từ vị trí đã lọc đến điểm mở khóa phải $\le 2.0$ mét trong $3$ chu kỳ liên tiếp, đồng thời vận tốc hướng tâm không được cho thấy người dùng đang rời xa, mới kích hoạt Relay. Sau khi cửa đã mở, hệ thống đi vào trạng thái nghỉ (Cooldown) để không lặp lại lệnh mở. Cửa chỉ quay về trạng thái khóa sẵn sàng phục vụ lần mở tiếp theo khi người dùng đã bước ra xa quá giới hạn $3.0$ mét. Nhờ thông tin vị trí hai chiều, cổng vận tốc hướng tâm còn giúp phân biệt người dùng \textit{thực sự tiến về cửa tài xế} với người \textit{vô tình đi ngang qua vùng 2 mét}, triệt tiêu hiện tượng mở cửa sai (False Positive). Logic này tạo ra trải nghiệm \textit{Passive Keyless Entry} mượt mà và an toàn.
...
\begin{enumerate}
    ...
    \item \textbf{Quyết định truy cập thông minh (AI-driven Decision):} Thay vì chỉ dựa vào ngưỡng khoảng cách vô hướng, log thực thi cho thấy lệnh mở GPIO rơ-le chỉ được phát ra khi đồng thời thỏa mãn: Khoảng cách Euclid từ vị trí $(x, y)$ đến điểm mở khóa $\le 2.0$m, cổng vận tốc hướng tâm xác nhận người dùng đang tiến lại gần VÀ xác suất $P_{walk}$ đạt ngưỡng tin cậy cao. Điều này minh chứng cho khả năng chống mở cửa sai (False Positive) khi người dùng chỉ đứng lảng vảng gần xe.
\end{enumerate}
