...
Nhìn chung, kết quả đạt được chứng minh hệ thống không chỉ phù hợp với mục tiêu nghiên cứu học thuật mà còn có tiềm năng phát triển thành giải pháp thực tế cấp độ công nghiệp (Industrial-grade), làm tiền đề lý tưởng để triển khai các thuật toán học máy (Machine Learning) nhận diện ý định hành vi ở giai đoạn 2 (đang hiện thực).

\par\noindent
\textbf{Source code của dự án:} 
\href{https://github.com/NhanVo-tech/Thesis.git}{\textcolor{blue}{\textbf{Kho mã nguồn trên GitHub}}}

\par\noindent
\textbf{Video demo hệ thống:} 
\href{https://drive.google.com/drive/folders/1kUymyDruULcvT3Kjg8Sff-fCjvxDySlZ?usp=drive_link}{\textcolor{red}{\textbf{Demo hệ thống}}}
...
\begin{itemize}
    \item \textbf{Giao tiếp đa vô tuyến tốc độ cao:} Nhóm đã cấu hình thành công kênh UART2 (HSU 115200 bps) cho module NFC (PN532) và kênh \textbf{USB-CDC} bản địa để nhận luồng khung \texttt{RANGE} từ cầu nối PC. Thiết kế này tách bạch hoàn toàn tải xử lý UWB khỏi vi điều khiển, loại bỏ hiện tượng nghẽn cổ chai vật lý khi đọc thẻ NFC hoặc nhận luồng dữ liệu Ranging liên tục.
    ...
    \begin{itemize}
        \item \textit{Core Task (Priority High):} Duy trì quảng bá BLE, xử lý NFC và phân tích khung \texttt{RANGE} của UWB để đảm bảo phản hồi nhanh nhất.
    ...
    \item \textbf{Định vị Trilateration, phân vùng Geofencing và máy trạng thái truy cập:} Module \texttt{Trilateration} hợp nhất ba khoảng cách thành tọa độ $(x, y)$, bộ lọc Kalman hai chiều (EKF) làm mượt quỹ đạo và ước lượng vận tốc $(v_x, v_y)$. Module \texttt{Geofence} phân lớp vị trí thành các vùng ảo (\texttt{DRIVER\_SEAT}, \texttt{DRIVER\_DOOR}, \texttt{WELCOME}, \texttt{OUTSIDE}) với bộ đệm Hysteresis chống dao động biên. Trên nền các vùng này, module \texttt{AccessController} triển khai máy trạng thái ba trạng thái (\texttt{LOCKED}, \texttt{DOOR\_UNLOCKED}, \texttt{OCCUPIED}): mở khóa chỉ khi người dùng \textbf{giảm tốc gần như dừng} ($<0.4$ m/s) gần cửa trong 3 chu kỳ liên tiếp (cổng giảm tốc triệt tiêu người đi ngang qua), khóa cửa và cấp quyền khởi động khi người dùng đứng yên ở ghế lái 3 giây, rồi khóa xe và thu hồi quyền khi người dùng rời đi. Logic tất định này đảm bảo rơ-le không bị đóng/ngắt liên tục và loại bỏ mở khóa sai (False Positive).
\end{itemize}
...
\begin{table}[!htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|L|c|L|}
        ...
        \hline
        1 & Nhận lệnh RANGING\_START \& Chờ Hệ điều hành của điện thoại & $\approx 584$ ms & Thời gian chờ hệ điều hành (Android) khởi động chip UWB, cấp phát tài nguyên dịch vụ FiRa/CCC và chuẩn bị ăng-ten. \\
        \hline
        2 & ESP32 phát lệnh điều khiển \& cầu nối PC khởi tạo phiên & $\approx 30$ ms & ESP32 gửi \texttt{CMD:START\_RANGING} qua USB-CDC; cầu nối PC thiết lập phiên DS-TWR trên ba mỏ neo. \\
        \hline
        3 & Bắt khung dữ liệu RANGE đầu tiên (Time-to-First-Fix) & $\approx 16$ ms & Ba mỏ neo hoàn tất khởi động, bắt sóng từ điện thoại và cầu nối PC trả về khung \texttt{RANGE} đầu tiên. \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|p{6cm}|c|}
        ...
        \hline
        Active (Ranging) & Đo UWB liên tục (ba mỏ neo). & $\approx 140$ mA \\
        \hline
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Độ ổn định của quỹ đạo sau khi áp dụng Trilateration và Kalman Filter:} Dữ liệu khoảng cách gốc từ ba mỏ neo DWM3001CDK thường tồn tại các dao động nhiễu tĩnh ($\pm 10\sim20$ cm) hoặc các bước nhảy vọt do hiện tượng phản xạ đa đường (multipath), dẫn đến tọa độ $(x, y)$ thô từ Trilateration bị rung lắc. Nhóm đã tích hợp thành công \textbf{Bộ lọc Kalman hai chiều (EKF)} ngay sau tầng Trilateration (nhiễu đo $R$ được dẫn xuất trực tiếp từ chất lượng lời giải RMS, cường độ nhiễu gia tốc $\sigma_a = 1.2$ m/s$^2$). Kết quả cho thấy đường cong quỹ đạo (trajectory) của người dùng trở nên cực kỳ mượt mà, đồng thời ước lượng được vector vận tốc $(v_x, v_y)$ phục vụ cổng vận tốc hướng tâm. Kỹ thuật này không chỉ giúp bộ đệm Hysteresis ra quyết định mở cửa chính xác tuyệt đối mà còn tạo ra chuỗi trạng thái $(x, y, v_x, v_y)$ chất lượng cao, phục vụ làm đầu vào (Input feature) cho mô hình nhận diện ý định hành vi ở giai đoạn 2 (đang hiện thực).
\end{itemize}

\subsubsection{Đánh giá định lượng bộ lọc Kalman (EKF)}

Để lượng hóa hiệu quả làm mượt của bộ lọc, nhóm sử dụng công cụ phân tích ngoại tuyến \texttt{analyze\_ekf.py} (đọc trực tiếp các dòng log \texttt{[POS2D]} và \texttt{[EKF]} do firmware phát ra) và đo trên hai kịch bản: \textbf{tag đứng yên} (stationary) và \textbf{người đi bộ quanh xe} (walking). Các chỉ số gồm giá trị trung bình, độ lệch chuẩn $\sigma$ (đo mức dao động), độ lệch cực đại và tỷ số giảm nhiễu $\sigma_{raw} / \sigma_{EKF}$.

\textbf{(a) Kịch bản tag đứng yên:} tag được đặt cố định gần xe trong một khoảng thời gian ngắn. Hình \ref{fig:ekf_noise_x} và \ref{fig:ekf_noise_y} biểu diễn chuỗi tọa độ thô (chấm xanh) đối chiếu với ngõ ra đã lọc (đường đỏ) theo trục $x$ và $y$; vùng tô màu thể hiện dải $\pm\sigma$ của từng chuỗi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/ekf_static_1_x_noise.png}
    \caption{Nhiễu tọa độ trục $x$ --- tag đứng yên (thô so với EKF)}
    \label{fig:ekf_noise_x}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/ekf_static_1_y_noise.png}
    \caption{Nhiễu tọa độ trục $y$ --- tag đứng yên (thô so với EKF)}
    \label{fig:ekf_noise_y}
\end{figure}
...
\begin{table}[H]
    ...
    \caption{Chỉ số làm mượt của EKF --- kịch bản tag đứng yên}
    \label{tab:ekf_static}
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        \textbf{Chỉ số} & \textbf{X (thô)} & \textbf{X (EKF)} & \textbf{Y (thô)} & \textbf{Y (EKF)} \\
        \hline
        Số mẫu & 132 & 280 & 132 & 280 \\
        \hline
        Trung bình (m) & $-1.597$ & $-1.597$ & $-1.123$ & $-1.123$ \\
        \hline
        Độ lệch chuẩn (cm) & $1.7$ & $1.4$ & $1.6$ & $1.5$ \\
        \hline
        Độ lệch cực đại (cm) & $13.7$ & $13.7$ & $8.3$ & $8.3$ \\
        \hline
        Tỷ số giảm nhiễu & \multicolumn{2}{c|}{$1.17\times$} & \multicolumn{2}{c|}{$1.11\times$} \\
        \hline
    \end{tabular}
\end{table}

Ở kịch bản này, bản thân ngõ ra Trilateration đã rất ổn định (độ lệch chuẩn chỉ $1.6$--$1.7$ cm, thấp hơn nhiều so với ngưỡng rung lắc $\pm10$--$20$ cm thường gặp), nhờ việc bù sai số hệ thống $24$ cm và tinh chỉnh Gauss-Newton. Bộ lọc EKF tiếp tục ép độ lệch chuẩn xuống còn $1.4$--$1.5$ cm (giảm nhiễu $\sim 1.1\times$), đồng thời cung cấp thêm vector vận tốc $(v_x, v_y)$ --- thông tin mà riêng tầng Trilateration không thể cung cấp. Giá trị trung bình gần như không đổi giữa hai chuỗi ($-1.597$ m và $-1.123$ m) chứng tỏ bộ lọc \textbf{không gây lệch (bias)} lên vị trí ước lượng.

\textbf{(b) Kịch bản người đi bộ quanh xe:} người dùng di chuyển quanh xe để đánh giá khả năng bám quỹ đạo. Hình \ref{fig:ekf_trajectory} cho thấy đường đi đã lọc (màu đỏ) bám sát các điểm đo thô (màu xanh) cùng vòng tròn bán kính mở khóa 2 m.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{fig/ekf_walk_1_trajectory.png}
    \caption{Quỹ đạo 2D --- các điểm đo thô đối chiếu với đường đi sau EKF}
...
\begin{table}[H]
    ...
    \renewcommand{\arraystretch}{1.25}
    \setlength{\tabcolsep}{4pt}
    \caption{Chỉ số bám quỹ đạo của EKF --- kịch bản người đi bộ}
    \label{tab:ekf_walk}
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        \textbf{Chỉ số} & \textbf{X (thô)} & \textbf{X (EKF)} & \textbf{Y (thô)} & \textbf{Y (EKF)} \\
        \hline
        Số mẫu & 150 & 331 & 150 & 331 \\
        \hline
        Trung bình (m) & $-0.454$ & $-0.499$ & $-0.544$ & $-0.515$ \\
        \hline
        Độ lệch chuẩn (cm) & $123.3$ & $122.0$ & $193.3$ & $194.0$ \\
        \hline
        Độ lệch cực đại (cm) & $196.4$ & $200.9$ & $315.4$ & $312.5$ \\
        \hline
        Tỷ số giảm nhiễu & \multicolumn{2}{c|}{$1.01\times$} & \multicolumn{2}{c|}{$1.00\times$} \\
        \hline
    \end{tabular}
\end{table}

Trong kịch bản đi bộ, độ lệch chuẩn lớn ($\sim 1.2$--$1.9$ m) phản ánh \textbf{biên độ dịch chuyển thực} của người dùng chứ không phải nhiễu. Việc tỷ số giảm nhiễu xấp xỉ $1.0\times$ là kết quả mong muốn: bộ lọc không ``nuốt'' chuyển động thật, quỹ đạo đã lọc gần như trùng khít với đường đi thực (không bị trễ --- lag), qua đó đảm bảo vector vận tốc $(v_x, v_y)$ ước lượng phản ánh trung thực động học người dùng, phục vụ chính xác cổng giảm tốc và cổng vận tốc hướng tâm của máy trạng thái truy cập. Sự dịch nhẹ của giá trị trung bình giữa hai trục là do phân bố không đối xứng của tuyến đường thử nghiệm, không phải sai số hệ thống.
...
% (walk/loiter/attack, 84.3%, P(Walk)/P(Loiter)/P(Attack)) — thay thế bởi
% phần Stage-2 intent recognition ở cuối khối \iffalse ... \fi này.
% =====================================================================
\iffalse
...
\fi
% =====================================================================
% [STAGE-2 AI - 2D INTENT RECOGNITION] KẾT QUẢ (đang hiện thực)
% =====================================================================

\subsection{Ứng dụng TinyML trong nhận diện ý định hành vi (đang hiện thực)}

\textit{Ghi chú:} Phần nhận diện ý định hành vi bằng TinyML ở giai đoạn 2 hiện \textbf{đang trong quá trình hiện thực}. Nội dung dưới đây trình bày quy trình thiết kế và kết quả dự kiến; số liệu thực nghiệm sẽ được cập nhật khi hoàn tất.

\begin{itemize}
    \item \textbf{Xây dựng bộ dữ liệu quỹ đạo (Dataset \& Feature Engineering):} Bộ dữ liệu gồm các chuỗi quỹ đạo $(x, y, v_x, v_y)$ được gán nhãn theo 4 lớp: \textit{Approach} (tiếp cận), \textit{Seated} (ngồi ghế lái), \textit{Leave} (rời đi) và \textit{Passing} (đi ngang qua). Dữ liệu được thu thập qua \texttt{localization\_demo.py} (10--15 lần chạy mỗi lớp), chuyển thành CSV có nhãn bằng \texttt{collect\_traj.py}, rồi chia tập huấn luyện/kiểm thử \textbf{theo từng lần chạy} (run-stratified) để tránh rò rỉ dữ liệu.

    \item \textbf{Kiến trúc mô hình tối ưu cho phần cứng nhúng:} Mạng nơ-ron tích chập một chiều Conv1D (cùng họ kiến trúc đã được kiểm chứng chạy trên TFLM) nhận đầu vào kích thước $(25 \times 4)$ và xuất 4 xác suất qua Softmax. Mô hình được huấn luyện trên Google Colab với chuẩn hóa z-score, sau đó xuất file header \texttt{uwb\_traj\_model.h} cùng tham số scaler để nhúng vào firmware.

    \item \textbf{Kết quả dự kiến (target):}
    \begin{itemize}
        \item \textit{Độ chính xác:} đạt tối thiểu $\sim 77\%$ trên tập kiểm thử độc lập, với ma trận nhầm lẫn được rà soát (ô nhiễu lớn nhất kỳ vọng là \texttt{passing} $\leftrightarrow$ \texttt{approach}).
        \item \textit{Hiệu năng phần cứng:} arena TFLM khởi đầu 35 KB (mở rộng tới $\sim 48$ KB nếu cần), suy luận trong vài mili-giây mỗi chu kỳ 100 ms, không ảnh hưởng tính thời gian thực của các tác vụ FreeRTOS.
    \end{itemize}

    \item \textbf{Kịch bản kiểm thử dự kiến:}
    \begin{itemize}
        \item \textit{Tiếp cận:} người dùng tiến lại gần cửa và giảm tốc dừng lại --- kỳ vọng $p_{\text{approach}} \ge 0.80$, cho phép mở khóa.
        \item \textit{Định cư ghế lái:} người dùng đứng yên tại ghế lái --- kỳ vọng $p_{\text{seated}} \ge 0.80$, khóa cửa và cấp quyền khởi động.
        \item \textit{Rời đi:} người dùng rời xa xe --- kỳ vọng $p_{\text{leave}} \ge 0.80$, khóa xe và thu hồi quyền.
        \item \textit{Đi ngang qua:} người dùng đi ngang mà không dừng --- kỳ vọng $p_{\text{passing}} \ge 0.70$ chặn cứng lệnh mở khóa (zero false-positive).
    \end{itemize}
\end{itemize}
