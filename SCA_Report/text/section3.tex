...
\begin{itemize}
    ...
    \item \textbf{Door Unlock Module (Geometric Hysteresis Logic):} Đảm nhiệm quyết định mở cửa. Thuật toán yêu cầu khoảng cách Euclid từ vị trí $(x, y)$ đến điểm mở khóa phải $\le 2.0$m trong 3 chu kỳ liên tiếp (Debounce) kết hợp cổng vận tốc hướng tâm (approach gate, từ chối người đang rời xa), mới kích hoạt GPIO, và chỉ reset khi điện thoại ra khỏi vùng an toàn ($> 3.0$m).
...
\begin{enumerate}
    ...
    Vị trí $(x, y)$ sau EKF được đẩy vào khối quyết định truy cập \texttt{AccessController}. Nếu khoảng cách Euclid đến điểm mở khóa cửa tài xế $< 2.0$m trong 3 chu kỳ liên tiếp, cổng vận tốc hướng tâm xác nhận người dùng đang tiến lại gần \textbf{VÀ} FSM đang ở trạng thái \texttt{AUTH\_SECURE\_CHANNEL\_READY} (Đã xác thực BLE), ESP32 mới cấp điện cho GPIO kích hoạt Relay mở cửa.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{UNLOCKING\_CHECK\_PROXIMITY}: Module \texttt{AccessController} thực hiện kiểm tra Hysteresis hình học trên vị trí $(x, y)$. Nếu khoảng cách Euclid đến điểm mở khóa thỏa mãn điều kiện an toàn (ví dụ: $\le 2.0$m trong 3 chu kỳ liên tiếp, kết hợp cổng vận tốc hướng tâm), kích hoạt sự kiện \texttt{PROXIMITY\_OK}.
