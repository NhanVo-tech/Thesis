...
bool UwbBridge::parseRange(const char* line, RangingFrame* out) {
    const char* p = line + 6;              // skip "RANGE:"
    double d0 = 0.0, d1 = 0.0, d2 = 0.0; int valid = 0;
    if (sscanf(p, "d0=%lf,d1=%lf,d2=%lf,valid=%d",
               &d0, &d1, &d2, &valid) != 4) return false;
    out->t_ms = millis();
    out->d[0] = d0; out->d[1] = d1; out->d[2] = d2;
    // Per-anchor mask: a positive distance marks a fresh, in-bounds anchor.
    out->valid_mask = 0;
...
Mặt nạ \texttt{valid\_mask} được xây dựng \textbf{theo từng mỏ neo}: một khoảng cách dương đồng nghĩa với một mỏ neo tươi và nằm trong phạm vi hợp lệ. Nhờ đó, khi một mỏ neo bị mất gói tin hoặc rơi ra ngoài vùng đo, hệ thống vẫn tiếp tục định vị bằng \textbf{đường suy biến hai mỏ neo} (2-anchor fallback) thay vì hủy bỏ toàn bộ khung dữ liệu. Với mỗi khung hợp lệ, ESP32 tiến hành định vị hai bước: \texttt{Trilateration::solve} trả về tọa độ $(x, y)$ kèm chất lượng lời giải RMS, sau đó \texttt{Ekf::update} hợp nhất phép đo vào quỹ đạo:
...
//    (nhieu do do ~ chat luong loi giai RMS)
Ekf::update(r.x, r.y, f.t_ms, r.rms);
\end{lstlisting}
\end{minipage}

Bộ lọc Kalman hai chiều nhận mỗi phép đo Trilateration làm một lần cập nhật: trước mỗi phép đo, nó dự báo trạng thái tới đúng thời điểm của khung dữ liệu ($\Delta t$ lấy từ dấu thời gian thực), rồi hiệu chỉnh bằng phép đo vị trí có nhiễu $\sigma_{meas} = \operatorname{clamp}(\text{RMS},\,0.05,\,1.0)$. Khi sóng đo bị gián đoạn, vị trí được giữ nguyên tại giá trị đã lọc cuối cùng (không chạy đà); nếu gián đoạn quá 2 giây, quỹ đạo được khởi tạo lại.
